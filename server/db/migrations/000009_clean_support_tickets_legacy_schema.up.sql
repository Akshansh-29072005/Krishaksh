-- Migrate legacy support_tickets schema from old `message` field to current `description` field.
-- This keeps legacy data, then removes the old field so inserts work correctly.

ALTER TABLE support_tickets ADD COLUMN IF NOT EXISTS title VARCHAR(200) NOT NULL DEFAULT 'Support Request';
...
-- Preserve legacy message data by copying it into description if needed.
UPDATE support_tickets
SET description = message
WHERE (description IS NULL OR description = '')
  AND message IS NOT NULL;

-- Set default values for any rows missing data.
UPDATE support_tickets SET title = 'Support Request' WHERE title IS NULL OR title = '';
...
-- Enforce non-null constraints for current schema.
